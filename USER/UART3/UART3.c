...
//{      
//	while((DEBUG_USART3->SR&0X40)==0);//循环发送,直到发送完毕   
//    DEBUG_USART3->DR = (u8) ch;      
//	return ch;
//}
//#endif
///*使用microLib的方法*/
...
//}
//*/
unsigned char  TxBuffer3[13];    //发送数据缓存区
u8 RxBuffer3[11] = {0};    //接收数据缓存区
uint8_t rec_flag3 = 0;
uint8_t tx_flag3 = 0;
__IO uint8_t TxCounter3 = 0x00;
__IO uint8_t RxCounter3 = 0x00; 
   void USART3_NVIC_Configuration()
	 {
   ...
   void USART3_GPIO_Configuration()
	 {
       ...
       //USART_ClearFlag(USART2,USART_FLAG_TC);  
			 USART_ITConfig(DEBUG_USART3,USART_IT_RXNE,ENABLE);//开启接收中断
...
//{
//  unsigned int i;
//		if(USART_GetFlagStatus(DEBUG_USART3,USART_FLAG_ORE)==SET)//溢出
//		{
//			USART_ReceiveData(DEBUG_USART3);                      //读DR
//			USART_ClearFlag(DEBUG_USART3,USART_FLAG_ORE);        //读SR
//		}
//  if(USART_GetITStatus(DEBUG_USART3, USART_IT_RXNE) != RESET)	   //判断读寄存器是否非空  接收中断
//  {	
//    
//    RxBuffer3[RxCounter3++] = USART_ReceiveData(DEBUG_USART3);             //将读寄存器的数据缓存到接收缓冲区里  读取接收到的数据
...
//  }
//  
//  if(USART_GetITStatus(DEBUG_USART3, USART_IT_TXE) != RESET)                   //这段是为了避免STM32 USART 第一个字节发不出去的BUG 
//  { 
//     USART_ITConfig(DEBUG_USART3, USART_IT_TXE, DISABLE);					            //禁止发缓冲器空中断， 
//  }	
//}
	 ...
	 void DEBUG_USART3_IRQHandler(void) 
	 {
		 ...
		 if(USART_GetITStatus(DEBUG_USART3, USART_IT_RXNE) != RESET)	   //判断读寄存器是否非空  接收中断
		 {
			 ...
			 rec_tem= USART_ReceiveData(DEBUG_USART3);  
			 Zigbee_IntGenData(rec_tem);
			  USART_ClearITPendingBit(DEBUG_USART3,USART_IT_RXNE);
		 }
//		 if(USART_GetITStatus(DEBUG_USART3, USART_IT_TXE) != RESET)                   //这段是为了避免STM32 USART 第一个字节发不出去的BUG 
//  { 
//     USART_ITConfig(DEBUG_USART3, USART_IT_TXE, DISABLE);					            //禁止发缓冲器空中断， 
...
void USART3_Send_Len_Str(u8 *str, u8 len)
{
	...
	for(i=0; i<len; i++)
		Usart3_SendByte(DEBUG_USART3,*str ++);
}

  /* 指令输入提示信息 */
 void Usart3_Show_Message(void)
{
	Usart3_SendString(DEBUG_USART3,"这是一个通过串口通信指令控制LED灯实验 \n");
	Usart3_SendString(DEBUG_USART3,"使用  USART  参数为：%d 8-N-1 \n");
	Usart3_SendString(DEBUG_USART3,"开发板接到指令后控制LED灯颜色，指令对应如下：\n");
	Usart3_SendString(DEBUG_USART3,"  指令   ------ LED灯号 \n");
	Usart3_SendString(DEBUG_USART3,"    1    ------    LED1 \n");
	Usart3_SendString(DEBUG_USART3,"     2    ------    BEEP \n");
  Usart3_SendString(DEBUG_USART3,"     3    ------    灭 \n");
